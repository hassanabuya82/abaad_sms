...
def course_registration(request):
    if request.method == 'POST':
        ...
        courses = Course.objects.filter(program__pk=student.department.id,  semester=current_semester
        ).exclude(id__in=t).order_by('year')
        all_courses = Course.objects.filter( program__pk=student.department.id)
...
def user_course_list(request):
    if request.user.is_lecturer:
        ...
        courses = Course.objects.filter(program__pk=student.department.id)
